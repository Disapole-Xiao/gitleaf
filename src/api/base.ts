/**
 * GitLeaf API - Base HTTP client for Overleaf
...
// Identify the actual extension, not node-fetch's generic default (which
// Overleaf rejected on history diffs). Read the shipped version, never spoof a browser.
const USER_AGENT = `GitLeaf/${(require('../../package.json') as { version: string }).version}`;
...
export interface HistoryUpdate {
    ...
    meta: {
        ...
        origin?:
            | { kind: 'file-restore'; path: string; timestamp: number; version: number }
            | { kind: 'project-restore'; timestamp: number; version: number }
            | { kind: 'dropbox' | 'upload' | 'git-bridge' | 'github' | 'history-resync' | 'history-migration' };
    };
    pathnames: string[];
    project_ops?: Array<{
        atV: number;
...
export class BaseAPI {
    ...
    constructor(url: string, private readonly historyLog: (message: string) => void = () => {}) {
    ...
    /** The account settings page exposes the signed-in user's profile as JSON. */
    private async getUserName(cookies: string, userId: string): Promise<string> {
        const res = await fetch(this.url + 'user/settings', {
            method: 'GET', redirect: 'manual', agent: this.agent,
            headers: { Cookie: cookies },
        });
        if (!res.ok) throw new Error('Could not read the Overleaf account name.');
        const body = await res.text();
        const value = body.match(/<meta\b(?=[^>]*\bname=["']ol-user["'])[^>]*\bcontent=(["'])(.*?)\1/i)?.[2];
        if (!value) throw new Error('Overleaf did not provide an account name.');
        const json = value.replace(/&(?:amp|quot|lt|gt|apos|#(?:x[0-9a-f]+|\d+));/gi, entity => {
            const key = entity.slice(1, -1).toLowerCase();
            const named: Record<string, string> = { amp: '&', quot: '"', lt: '<', gt: '>', apos: "'" };
            if (key in named) return named[key];
            const code = key[1] === 'x' ? parseInt(key.slice(2), 16) : parseInt(key.slice(1), 10);
            return String.fromCodePoint(code);
        });
        const user = JSON.parse(json) as { id?: string; _id?: string; first_name?: string; last_name?: string };
        if ((user.id ?? user._id) !== userId) throw new Error('Overleaf account identity changed during login.');
        const name = [user.first_name, user.last_name].filter(Boolean).join(' ').trim();
        if (!name) throw new Error('Overleaf account has no display name.');
        return name;
    }
    ...
    async cookiesLogin(cookies: string): Promise<ResponseSchema> {
        ...
        if (res) {
            ...
            const userName = await this.getUserName(cookies, userId);
    ...
    private async getJson<T>(route: string, body?: object, noContent = false): Promise<T> {
        if (!this.identity) throw new Error('Not authenticated');
        const started = Date.now();
        const res = await fetch(this.url + route, {
            redirect: 'manual', agent: this.agent, timeout: 15000,
            method: body ? 'POST' : 'GET',
            headers: { Cookie: this.identity.cookies, Accept: 'application/json', 'Content-Type': 'application/json', 'X-CSRF-Token': this.identity.csrfToken, 'User-Agent': USER_AGENT },
            body: body ? JSON.stringify({ _csrf: this.identity.csrfToken, ...body }) : undefined,
        });
        // Never log cookies, response bodies, or the query (which contains filenames).
        this.historyLog(`History ${body ? 'POST' : 'GET'} ${route.split('?')[0].replace(/project\/[^/]+/, 'project/:id')} → ${res.status} (${Date.now() - started}ms)`);
        if (![200, 201, 204].includes(res.status)) {
            // Consume the response for keep-alive reuse without logging its
            // body: an authentication response can contain sensitive data.
            const errorBody = await res.text();
            this.historyLog(`History response: ${historyResponseDiagnostic(res.headers, errorBody)}`);
            throw new HistoryRequestError(res.status, res.status === 429 ? historyRetryAt(res.headers.get('retry-after')) : undefined);
        }
        if (res.status === 204 || noContent) { await res.text(); return undefined as T; }
        return await res.json() as T;
    }

    flushHistory(project: string): Promise<void> {
        // The web proxy may turn the history service's 204 into an empty 200.
        // Flush has no JSON response contract; do not attempt to parse it.
        return this.getJson(`project/${encodeURIComponent(project)}/flush`, {}, true);
    }

    labelHistory(project: string, version: number, comment: string): Promise<HistoryLabel> {
        return this.getJson(`project/${encodeURIComponent(project)}/labels`, { version, comment });
    }

    restoreHistory(project: string, version: number): Promise<unknown> {
    ...
    getHistory(project: string, before?: number): Promise<HistoryPage> {
        ...
        return this.getJson(`project/${encodeURIComponent(project)}/updates?${query}`);
    }

    getHistoryFiles(project: string, from: number, to: number): Promise<{ diff: HistoryFile[] }> {
        return this.getJson(`project/${encodeURIComponent(project)}/filetree/diff?from=${from}&to=${to}`);
    }

    getHistoryDiff(project: string, file: string, from: number, to: number): Promise<HistoryDiff> {
        const query = new URLSearchParams({ pathname: file, from: String(from), to: String(to) });
        return this.getJson(`project/${encodeURIComponent(project)}/diff?${query}`);
    }
        ...
        if (result) {
            return {
                ...
                userInfo: { userId: result.userId, userEmail: result.userEmail,
                    userName: await this.getUserName(this.identity.cookies, result.userId) },
            };
        }
        return {
            type: 'error',
