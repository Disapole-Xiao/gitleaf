/**
 * GitLeaf Socket.io API - Real-time sync with Overleaf
 * Adapted from Overleaf-Workshop
 */

import { promisify } from 'util';
import { BaseAPI, ProjectEntity, FileEntity } from './base';
import { Identity } from '../core/credentials';
...
export function setOutputChannel(channel: { appendLine(message: string): void }) {
...
export class SocketIOAPI {
    ...
    private setupInternalHandlers() {
        ...
        this.socket.on('forceDisconnect', (message: string, _delay: number = 10) => {
    ...
    async applyOtUpdate(docId: string, update: DocumentUpdate): Promise<void> {
        await new Promise<void>((resolve, reject) => {
            const finish = (error?: Error) => {
                clearTimeout(timer);
                this.socket.removeListener('otUpdateApplied', acknowledge);
                this.socket.removeListener('disconnect', disconnected);
                if (error) reject(error); else resolve();
            };
            const acknowledge = (message: DocumentUpdate) => {
                const own = message.op === undefined || (!!message.meta?.source && message.meta.source === this.publicId);
                if (message.doc === docId && own && message.v >= update.v) finish();
            };
            const disconnected = () => finish(new Error('Disconnected before the document update was acknowledged.'));
            const timer = setTimeout(() => finish(new Error('Document update acknowledgement timed out.')), 15000);
            this.socket.on('otUpdateApplied', acknowledge);
            this.socket.on('disconnect', disconnected);
            // The callback only acknowledges queueing. Wait for the document
            // updater's versioned event before declaring a push successful.
            void this.emit('applyOtUpdate', docId, update).catch(error => finish(error));
        });
    }
